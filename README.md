...
> _c就是我们上一篇文章提到的 createElement 函数 。
>
> 执行createElement函数可以方便我们生成 vnode的。
>
...
原理解析： 
 ...
 1. 每当我们遇到`<`时，会检查是否是开始标签```（如<div> 的 <）```或者闭合标签的前标签```如（</div>）的<```： 

 2. 如果是开始标签，则会解析该标签的内容。然后将解析该标签获得的标签名、属性等相关信息存储在对象中，然后将其堆栈中（ ```1.如果此时 ast对象为空，则表示是第一次解析。直接将解析出来的对象赋值给 ast对象。2.如果此时 ast对象不为空，则表示正处于当前标签的解析过程中，这个时候咱不处理。```）。因为此时还没遇到闭合标签，所以表示该标签还没有解析完毕。我们需要继续解析。当遇到一个开始标签时，重复步骤二。当遇到一个闭合标签是，重复步骤三。

 3. 如果是闭合标签，则会解析该标签的标签名。```然后看是否和栈顶元素保持一致。如果一致，则将栈顶元素存放在他父元素的 children属性下，然后直接移除栈顶元素。```此时标签的解析权移交给了他的上一个元素，即他的父元素。然后继续解析，当遇到一个开始标签时，重复步骤一。当遇到一个闭合标签是，重复步骤三。
...
其中html-parser文件中的parseHTML函数是专门用来解析 html模版的。
...
> 本文中实现的模版编译没有考虑异常情况以及指令、data等。后续我们会慢慢补充。
